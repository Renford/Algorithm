...
最小平衡二叉树的节点数：F(n)=F(n-1)+F(n-2)+1。平衡二叉树具有以下特点：
...
[Javascript代码看这里](../code/search/search_tree_avl.js)

高度平衡树，在AVL中任何节点的两个儿子子树的高度最大差别为1，查找、插入和删除在平均和最坏情况下都是O(log n)。增加和删除可能需要通过一次或多次树旋转来重新平衡这个树。

#### 红黑树

[Javascript代码看这里](../code/search/search_tree_rb.js)

是一种自平衡二叉查找树，又称对称二叉B树，查找，插入和删除在平均和最坏情况下都是O(log n)。具有以下特点
+ 节点是红色或黑色
+ 根节点是黑色
+ 每个叶节点（NIL或空节点）是黑色的
+ 每个红色节点的两个子节点都是黑色。(从每个叶子到根的所有路径上不能有两个连续的红色节点)
+ 从任一节点到其每个叶子的所有路径都包含相同数目的黑色节点
+ 时间复杂度：插入和删除在最坏的情况下为 O(logn)
+ 红黑树的高度最多为 log(n + 1)
...
也叫分裂树，是一种自平衡二叉查找树，查找，插入和删除在平均和最坏情况下都是O(log n)。优势在于不需要记录用于平衡树的冗余信息。

+ 基本特点：不会保证树一直是平衡的，但各种操作的平摊时间复杂度是O(log n)
+ 伸展树的出发点：刚被访问的内容下次可能仍会被访问，查找次数多的内容可能下一次会被访问
+ 基本操作：每次查找节点之后对树进行重构，把被查找的节点搬移到树根

<!-- http://blog.csdn.net/u014634338/article/details/49586689 -->
<!-- https://www.cnblogs.com/yousiki/p/6147455.html -->
...
它是一种平衡的多叉树，m 阶的B树满足以下条件：

1. 根结点至少有两个子女；
2. 每个非根节点所包含的关键字个数 j 满足：Math.floor(m/2) - 1 <= j <= m - 1
3. 节点有 k 个关键码，则有 k+1 棵子树
4. 所有的叶子结点都位于同一层
...
2. 分支结点，每个结点包含分支数范围为[Math.floor(m/2), m]；
3. 节点有 k 个关键码，则有 k+1 棵子树
4. 所有叶子结点都在同一层；
